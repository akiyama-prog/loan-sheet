  ...
  const validator=(e)=> {
    switch (e.target.name) {
        ...
        case 'property_borrowing_amount':
            return requiredNumberValidation(e.target.value);
            break;
        //勤務先
        ...
        //保有不動産
        case 'building_use':
            return requiredJPValidation(e.target.value);
            break;
        //file
...
const requiredNumberValidation=(value)=> {
    const regex = /^[0-9]+$/
    if (!regex.test(value)) return '※半角数字で入力してください'
    return '';
}
const requiredJPValidation=(value)=> {
    const regex = /^[ぁ-んァ-ヶー一-龠]+$/
    if (!regex.test(value)) return '※正しい形式で入力してください'
    return '';
}
...
const fileValidation=(value)=> {
  ...
  useEffect(() => {
    let unsubscribe = auth.onAuthStateChanged((user) => {
        ...
        if (db.collection('customers').doc(auth.currentUser.uid)) {
        //編集中のdataをセット
        ...
        db.collection('customers').doc(auth.currentUser.uid).collection('informations').doc('houseInfo').get().then(ob => {
            if (Object.keys(ob.data()).length) {
                setHouseInfo(ob.data());
            }
        })
        db.collection('customers').doc(auth.currentUser.uid).collection('informations').doc('companyInfo').get().then(ob => {
            if (Object.keys(ob.data()).length) {
                setCompanyInfo(ob.data());
            }
        })
        db.collection('customers').doc(auth.currentUser.uid).collection('informations').doc('familyInfo').get().then(ob => {
            if (Object.keys(ob.data()).length) {
        ...
        db.collection('customers').doc(auth.currentUser.uid).collection('informations').doc('propertyInfo').get().then(ob => {
            if (Object.keys(ob.data()).length) {
        ...
        db.collection('customers').doc(auth.currentUser.uid).collection('informations').doc('financialAssets').get().then(ob => {
            if (Object.keys(ob.data()).length) {
    ...
    <div>
      ...
      <div className="container">
        <div className="text-center">
          <h1 className="pt-4 pb-4"><Link href="/edit">ローン相談表</Link></h1>
          <p>{customerInfo.name_kanji}様</p>
        </div>
